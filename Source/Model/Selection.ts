...
class Selection_
{
	...
	static buildFromStringSRT(selectionAsStringSRT: string): Selection_
	{
		...
		var returnValue = new Selection_
		(
			tag,
	...
	static buildManyFromStringSRT(selectionsAsStringSRT: string): Selection_[]
	{
		var returnValues = [];
		...
		for (var i = 0; i < selectionsAsStringsSRT.length; i++)
		{
			...
			{
				var selection = Selection_.buildFromStringSRT
				(
	...
	static convertManyToStringSRT(selections: Selection_[]): string
	{
		var returnValue = "";
	...
	overlapsWith(other: Selection_): boolean
	{
		return false; // todo
	}
	...
	toString(): string
	{
		var timeStartAsString =
			TimeHelper.secondsToStringSecondsMilliseconds
			(
	...
	toStringSRT(index: number): string
	{
